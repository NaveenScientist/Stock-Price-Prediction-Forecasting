...
# ----------------- CONFIGURATION -----------------
st.set_page_config(
    page_title="Nifty50 Pro Analytics",
    page_icon="📈",
    layout="wide",
...
# ----------------- CONSTANTS -----------------
RESULTS_FOLDER = r"D:\Datasets\Stock trend prediction datasets(N50)\Results_MultiFeature_7day"
...
def get_live_price(stock_name):
    """Fetch live price using yfinance with heuristic symbol mapping."""
    symbol_map = {
...
def load_summary(csv_path):
    ...
    if os.path.exists(mapping_file):
        ...
        manual_map = {
            'Adani Ent': 'Adani Enterprises Ltd.',
            'Adani Ports': 'Adani Ports and SEZ Ltd.',
            'Airtel': 'Bharti Airtel Ltd.',
            'Apollo Hosp': 'Apollo Hospitals Enterprise Ltd.',
            ...
            'HDFC bnk': 'HDFC Bank Ltd.',
            'HDFC Life': 'HDFC Life Insurance Company Ltd.',
            'Hero moto': 'Hero MotoCorp Ltd.',
            'Hindalco': 'Hindalco Industries Ltd.',
            'HUL': 'Hindustan Unilever Ltd.',
            'ICICI bnk': 'ICICI Bank Ltd.',
            'Indusind bnk': 'IndusInd Bank Ltd.',
            'Infosys': 'Infosys Ltd.',
            'ITC': 'ITC Ltd.',
            'JIO finance': 'Jio Financial Services Ltd.',
            'JSW steel': 'JSW Steel Ltd.',
            'Kotak mahindra bnk': 'Kotak Mahindra Bank Ltd.',
            'L&T': 'Larsen & Toubro Ltd.',
            'M&M': 'Mahindra & Mahindra Ltd.',
            'Maruti Suzuki': 'Maruti Suzuki India Ltd.',
            'Nestle ind': 'Nestle India Ltd.',
            'NTPC': 'NTPC Ltd.',
            'ONGC': 'Oil and Natural Gas Corporation Ltd.',
            'Power Grid': 'Power Grid Corporation of India Ltd.',
            'Reliance': 'Reliance Industries Ltd.',
            'SBI': 'State Bank of India',
            'SBI life': 'SBI Life Insurance Company Ltd.',
            ...
            'Tech M': 'Tech Mahindra Ltd.',
            'Titan': 'Titan Company Ltd.',
            'Trent': 'Trent Ltd.',
            'Ultra Tech cem': 'UltraTech Cement Ltd.',
            'Wipro': 'Wipro Ltd.'
        }
...
def main():
    ...
    if summary is None:
        st.error(f"Summary file not found at {SUMMARY_FILE}. Please check the path.")
        st.stop()
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.info("💡 **Tip:** Use the 'Stock Analysis' tab for detailed interactive charts.")
...
def render_dashboard(summary):
    ...
    fig = px.bar(
        ...
        labels={'R2_day1': 'Average R² Score'},
    ...
    top_stocks = summary.sort_values('R2_day1', ascending=False).head(5)[['Stock', 'Sector', 'R2_day1', 'RMSE_day1']]
    st.dataframe(
        top_stocks.style.background_gradient(subset=['R2_day1'], cmap='Greens'),
...
def render_stock_analysis(stock_name, summary):
    ...
    rmse = stock_data['RMSE_day1']
    next_day_pred = stock_data['Next_7_List'][0] if len(stock_data['Next_7_List']) > 0 else 0
    ...
    col2.metric("Model R² (Day 1)", f"{r2:.4f}")
    col3.metric("RMSE", f"{rmse:.2f}")
    
    if live_price:
        delta = live_price - next_day_pred
        col4.metric("Live Price", f"₹{live_price:.2f}", delta=f"{delta:.2f} vs Pred")
    else:
    ...
    with tab1:
        ...
        if history_df is not None:
            ...
            st.warning("Historical data file not found. Showing only forecast.")
            next7 = stock_data['Next_7_List']
            if next7:
                days = list(range(1, 8))
                fig = px.line(x=days, y=next7, markers=True, title="7-Day Price Forecast")
